...
public class QueueTest extends VertxTestBase {
  ...
  @Test
  public void testJuuSync() {
    testJuu(SYNC);
  }

  @Test
  public void testJuuAsync() {
    testJuu(ASYNC);
  }

  private void testJuu(Mode m) {
    Queue<String> queue = m.queue();
    AtomicInteger events = new AtomicInteger();
    queue.handler(s -> {
      m.checkThread();
  ...
  @Test
  public void testProducerFasterThanConsumeAsync() {
    Queue<String> queue = Queue.queue(vertx.getOrCreateContext(), 4);
    queue.writableHandler(v -> {});
    queue.handler(s -> {});
